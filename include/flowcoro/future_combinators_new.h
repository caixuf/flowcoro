...
public:
    ...
    void await_suspend(std::coroutine_handle<> handle) {
        ...
        auto start_task = [this](auto& task, size_t index) {
            GlobalThreadPool::enqueue([this, task_ref = std::ref(task), index]() mutable {
                try {
                    if constexpr (std::is_void_v<typename std::remove_reference_t<decltype(task_ref.get())>::value_type>) {
                        task_ref.get().get();
...
public:
    ...
    void await_suspend(std::coroutine_handle<> handle) {
        ...
        auto start_task = [this](auto& task, size_t task_index) {
            GlobalThreadPool::enqueue([shared_state = shared_state_, task_ref = std::ref(task), task_index]() mutable {
                try {
                    if constexpr (std::is_void_v<typename std::remove_reference_t<decltype(task_ref.get())>::value_type>) {
                        task_ref.get().get();
...
public:
    ...
    void await_suspend(std::coroutine_handle<> handle) {
        ...
        auto start_task = [this](auto& task, size_t task_index) {
            GlobalThreadPool::enqueue([this, task_ref = std::ref(task), task_index]() mutable {
                try {
                    if constexpr (std::is_void_v<typename std::remove_reference_t<decltype(task_ref.get())>::value_type>) {
                        task_ref.get().get();
                        ...
                        auto result = task_ref.get().get();
                        using TaskType = typename std::remove_reference_t<decltype(task_ref.get())>::value_type;
