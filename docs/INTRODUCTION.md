...
FlowCoro 是一个基于 **C++20 协程**的高性能异步编程库，专为批量任务处理和高吞吐量场景而设计。它将现代 C++ 协程技术与无锁数据结构相结合，提供了一套简洁而高效的并发编程接口。
...
- **完整的调度系统**：默认单个 CoroutineScheduler + 后台 ThreadPool
...
协程管理器 (CoroutineManager)
      ...
      │ 投递到协程池（默认单个调度器）
      ▼
协程池 (CoroutinePool) ── 1 个 CoroutineScheduler
      │
      │ 无锁队列；CPU/阻塞任务另走 ThreadPool
      ▼
线程池 (ThreadPool) ── 后台工作线程
```
...
3. **进入调度系统**：协程句柄进入默认那个调度器的无锁队列
4. **调度器线程恢复**：CoroutineScheduler 工作线程取出协程并 resume
5. **完成通知**：协程完成后唤醒等待它的上层协程

### 关键设计决策

- **无锁队列**：调度队列使用 lock-free 数据结构
- **CPU 亲和性**：调度器可绑定 CPU 核心
- **批量处理**：调度循环每次批量处理多个协程
- **空闲 park**：worker 空闲时 park，入队唤醒；不是不可打断的 sleep/yield
- **内存池**：协程帧使用专用内存池分配
...
2. **运行性能**：无锁架构 + 单调度器 + ThreadPool + 内存池
