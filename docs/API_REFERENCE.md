# FlowCoro v2.2.0 API 参考手册

## 🆕 v2.2.0 更新亮点

### 增强的Task生命周期管理
- **原子状态管理**: 使用atomic flags (`is_cancelled_`, `is_destroyed_`) 跟踪状态
- **线程安全保护**: 互斥锁保护关键状态变更操作  
- **安全销毁**: `safe_destroy()` 方法防止协程句柄损坏
- **Promise风格API**: JavaScript风格的状态查询接口

### 新增状态查询API
```cpp
// Promise风格状态查询
task.is_pending()    // 任务进行中 (类似JS Promise.pending)
task.is_settled()    // 任务已结束 (类似JS Promise.settled) 
task.is_fulfilled()  // 任务成功完成 (类似JS Promise.fulfilled)
task.is_rejected()   // 任务失败或取消 (类似JS Promise.rejected)
```
